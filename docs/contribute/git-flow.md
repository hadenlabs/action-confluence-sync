...
To follow the instructions in this guide and start contributing to action-docker-template project on Gitlab:
...
```bash
git remote add upstream  https://github.com/hadenlabs/action-docker-template.git
```

Now your **upstream** points to **hadenlabs/action-docker-template**.
...
All merge requests are automatically tested using [Github Actions](https://github.com/hadenlabs/action-docker-template/actions). In case some tests fail, fix the issues or describe why the fix cannot be done.
