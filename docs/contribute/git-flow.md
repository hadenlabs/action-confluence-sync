...
To contribute to action-confluence-sync project on [GitHub](https://github.com/hadenlabs/action-confluence-sync), We use Gitflow (See more [here](https://datasift.github.io/gitflow/IntroducingGitFlow.html)) In a nutshell, it means that you should branch from the main repository and contribute back by making [pull request](https://github.com/hadenlabs/action-confluence-sync/pulls).
...
```bash
git remote add upstream  https://github.com/hadenlabs/action-confluence-sync.git
```

Now your **upstream** points to **hadenlabs/action-confluence-sync**.
...
git fetch upstream # assuming upstream is hadenlabs/action-confluence-sync
...
All merge requests are automatically tested using [Github Actions](https://github.com/hadenlabs/action-confluence-sync/actions). In case some tests fail, fix the issues or describe why the fix cannot be done.
